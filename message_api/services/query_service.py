...
def load_history_on_startup():
    ...
    history_from_db = db_utils.load_history_from_db()
...
def _add_turn_to_history(user_content: str, assistant_content: str):
    ...
    db_utils.add_message_to_db(role="user", content=user_content, timestamp=now_utc.isoformat())
    _chat_history.append(ChatMessageSchemaObj(role="user", content=user_content, timestamp=now_utc))

    db_utils.add_message_to_db(role="assistant", content=assistant_content, timestamp=now_utc.isoformat())
